...
async fn main() -> Result<()> {
    ...
    // Загружаем .env ДО инициализации логирования, чтобы RUST_LOG из .env
    // действительно применялся (Config::load() делает это слишком поздно).
    let _ = dotenv::dotenv();
    ...
    tracing_subscriber::fmt()
        ...
        .with_env_filter(
            tracing_subscriber::EnvFilter::try_from_default_env()
                // ВАЖНО: раньше здесь стояло "xiaozhi_backend=info" — имя
                // крейта не совпадало (mediarise_robot_console), из-за чего ВСЕ
                // прикладные логи были выключены. Глобальный "info" чинит это.
